...
def download_file_from_github(filename):
    url = f"{BASE_RAW_URL}{filename}?t={int(time.time())}"
    try:
...
def wait_and_arrange_windows(target_count=None):
    ...
    # Структура для получения рабочей области экрана (исключая панель задач)
    ...
    work_area = RECT()
    # 48 это SPI_GETWORKAREA — возвращает координаты рабочей области без панели задач
    ...
    start_x = work_area.left
    start_y = work_area.top
    
    print(f"\n[Диагностика экрана] Рабочая зона (без панели задач): {screen_w}x{screen_h} (с точки X:{start_x}, Y:{start_y})")

    half_w = screen_w // 2
    half_h = screen_h // 2

    # Кастомная расстановка по углам с учетом отступов рабочей зоны:
    grid_positions = [
        {"x": start_x + half_w, "y": start_y, "w": screen_w - half_w, "h": half_h},                 # 1. Правый верх
        {"x": start_x, "y": start_y + half_h, "w": half_w, "h": screen_h - half_h},                 # 2. Левый низ
        {"x": start_x + half_w, "y": start_y + half_h, "w": screen_w - half_w, "h": screen_h - half_h}, # 3. Правый низ
        {"x": start_x, "y": start_y, "w": half_w, "h": half_h}                                  # 4. Левый верх
    ]
